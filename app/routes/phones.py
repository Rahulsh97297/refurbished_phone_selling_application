...
def create_phone():
    ...
    phone = Phone(**obj)
    # auto-price fill if missing
    overrides = {
        "X": obj.get("price_x"),
        "Y": obj.get("price_y"),
        "Z": obj.get("price_z"),
    }
    prices = recommend_prices(phone.base_price, bp.server.config["PLATFORM_FEES"], bp.server.config["PRICE_MIN_MARGIN"], overrides)
...
def update_phone(phone_id):
    ...
    # re-compute prices if base_price changed and overrides not supplied
    overrides = {
        "X": obj.get("price_x"),
        "Y": obj.get("price_y"),
        "Z": obj.get("price_z"),
    ...
    if any(k in obj for k in ("base_price", "price_x", "price_y", "price_z")):
        prices = recommend_prices(
            phone.base_price,
            bp.server.config["PLATFORM_FEES"],
            bp.server.config["PRICE_MIN_MARGIN"],
            overrides,
        )
        phone.price_x, phone.price_y, phone.price_z = prices["X"], prices["Y"], prices["Z"]
...
# Bulk CSV upload (UI form posts to this)
