...
def _to_positive_float(value) -> Optional[float]:
    """Coerce override to a positive float if possible; else None."""
    if value is None:
        return None
    try:
        f = float(value)
...
def recommend_prices(
    ...
    """
    Compute platform prices ensuring at least `min_margin` margin above base.
    If manual_overrides provided (e.g., {'X': 199.99}), use them when valid.
    Safely handles None/non-numeric overrides.
    """
    manual_overrides = manual_overrides or {}
    out = {}
    target_revenue = base_price * (1.0 + min_margin)
    ...
    for platform, cfg in fee_config.items():
        override = _to_positive_float(manual_overrides.get(platform))
        if override is not None:
            out[platform] = round(override, 2)
            continue
        rule = FeeRule(type=cfg["type"], value=cfg["value"], fixed=cfg.get("fixed", 0.0))
        out[platform] = apply_fee(target_revenue, rule)
    return out
